#!/usr/bin/env python3
"""
Generate thesis figures for SVM hyperparameter analysis.

This script creates box plots showing the effect of each SVM hyperparameter
...
# SVM hyperparameters to analyze
SVM_PARAMS = {
...
def load_mlflow_data(experiment_name: str = None) -> pd.DataFrame:
    ...
    for exp_id in experiment_ids:
        runs = mlflow.search_runs(
            ...
            filter_string="params.model_type = 'svm'",
            max_results=10000,
        )
        if not runs.empty:
    ...
    if not all_runs:
        print("No SVM runs found in MLflow")
        return pd.DataFrame()

    df = pd.concat(all_runs, ignore_index=True)
    print(f"Loaded {len(df)} SVM experiment runs")
    return df
...
def clean_and_prepare_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Filter for SVM experiments only
    df = df[df["params.model_type"] == "svm"].copy()

    if df.empty:
        print("No SVM experiments found")
        return df
    ...
    for idx, row in df.iterrows():
        ...
        for param in ["C", "kernel", "epsilon", "gamma", "embedding", "normalize"]:
    ...
    if "params.C" in df.columns:
        df["params.C"] = pd.to_numeric(df["params.C"], errors="coerce")
    if "params.epsilon" in df.columns:
        df["params.epsilon"] = pd.to_numeric(df["params.epsilon"], errors="coerce")
...
def create_consolidated_plot(
    ...
    if param in ["C", "epsilon"]:
        # Numeric parameters - sort numerically and use log scale for C
        all_param_values = sorted(embedding_data[param_col].dropna().unique())
        use_log_scale = param == "C"
    else:
        # Categorical parameters - use predefined order
        param_orders = {
            "kernel": ["linear", "rbf", "poly", "sigmoid"],
            "gamma": ["scale", "auto"],  # SVM gamma can only be 'scale' or 'auto'
        }
        if param in param_orders:
            # Filter to only include valid values for this parameter
            valid_values = param_orders[param]
            actual_values = embedding_data[param_col].dropna().unique()
            all_param_values = [v for v in valid_values if v in actual_values]
        else:
            all_param_values = sorted(embedding_data[param_col].dropna().unique())
        use_log_scale = False
    ...
    for i, param_value in enumerate(all_param_values):
        ...
        if len(no_norm_values) > 0:
            ...
            no_norm_stds.append(no_norm_values.std() if len(no_norm_values) > 1 else 0)
        else:
    ...
    if param in ["C", "epsilon"]:
        ...
        if use_log_scale and param == "C":
            ...
            # Use positions for linear scale
            if not all(np.isnan(norm_means)):
                ax.errorbar(
                    x_positions,
                    norm_means,
    ...
    ax.set_title(
        f"Effect of {SVM_PARAMS[param]} on {metric_name}\n{embedding_str} Embedding",
        fontsize=14,
        fontweight="bold",
    )
    ax.set_xlabel(SVM_PARAMS[param], fontsize=12)
    ...
    # Create filename
    filename = f"svm_{param}_{metric}_{embedding}_consolidated.png"
...
def create_summary_statistics(data: pd.DataFrame, output_dir: Path):
    ...
    if summary_stats:
        ...
        summary_df.to_csv(output_dir / "svm_summary_statistics.csv", index=False)
        print(f"Saved summary statistics: {len(summary_df)} rows")


def main():
    parser = argparse.ArgumentParser(
        description="Generate SVM hyperparameter analysis figures"
    )
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    print(f"Generating SVM analysis figures in: {output_dir}")
    ...
    print(
        ...
        f"{len(SVM_PARAMS)} parameters, {len(available_embeddings)} embeddings "
    ...
    print(f"Summary statistics saved to svm_summary_statistics.csv")

    # Print organization suggestion
    print(f"\n=== Thesis Organization Suggestion ===")
    for param in SVM_PARAMS.keys():
        print(f"\nSection: Effect of {SVM_PARAMS[param]} on Prediction Quality")
        for embedding in available_embeddings:
            embedding_name = (
                embedding.upper() if embedding == "tfidf" else embedding.capitalize()
            )
