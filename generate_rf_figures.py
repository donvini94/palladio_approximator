#!/usr/bin/env python3
"""
Generate thesis figures for Random Forest hyperparameter analysis.

This script creates plots showing the effect of each Random Forest hyperparameter
...
# Random Forest hyperparameters to analyze
RF_PARAMS = {
...
def load_mlflow_data(experiment_name: str = None) -> pd.DataFrame:
    ...
    for exp_id in experiment_ids:
        runs = mlflow.search_runs(
            ...
            filter_string="params.model_type = 'rf'",
            max_results=10000,
        )
        if not runs.empty:
    ...
    if not all_runs:
        print("No Random Forest runs found in MLflow")
        return pd.DataFrame()

    df = pd.concat(all_runs, ignore_index=True)
    print(f"Loaded {len(df)} Random Forest experiment runs")
    return df
...
def extract_hyperparameter_from_run_name(run_name: str) -> Dict[str, str]:
    ...
    # Parse run name format: param=value_model=rf_emb=embedding_norm=normalization_runX
...
def clean_and_prepare_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Filter for Random Forest experiments only
    df = df[df["params.model_type"] == "rf"].copy()

    if df.empty:
        print("No Random Forest experiments found")
        return df
    ...
    for idx, row in df.iterrows():
        ...
        for param in ["n_estimators", "max_depth", "embedding", "normalize"]:
    ...
    numeric_params = ["n_estimators", "max_depth"]
...
def create_consolidated_plot(
    ...
    # RF parameters are discrete, so use line plots with markers
    ...
    ax.set_title(
        f"Effect of {RF_PARAMS[param]} on {metric_name}\n{embedding_str} Embedding",
        fontsize=14,
        fontweight="bold",
    )
    ax.set_xlabel(RF_PARAMS[param], fontsize=12)
    ...
    # Create filename
    filename = f"rf_{param}_{metric}_{embedding}_consolidated.png"
...
def create_summary_statistics(data: pd.DataFrame, output_dir: Path):
    ...
    if summary_stats:
        ...
        summary_df.to_csv(output_dir / "rf_summary_statistics.csv", index=False)
        print(f"Saved summary statistics: {len(summary_df)} rows")


def main():
    parser = argparse.ArgumentParser(
        description="Generate Random Forest hyperparameter analysis figures"
    )
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    print(f"Generating Random Forest analysis figures in: {output_dir}")
    ...
    print(
        ...
        f"{len(RF_PARAMS)} parameters, {len(available_embeddings)} embeddings "
    ...
    print(f"Summary statistics saved to rf_summary_statistics.csv")

    # Print organization suggestion
    print(f"\n=== Thesis Organization Suggestion ===")
    for param in RF_PARAMS.keys():
        print(f"\nSection: Effect of {RF_PARAMS[param]} on Prediction Quality")
        for embedding in available_embeddings:
            embedding_name = (
                embedding.upper() if embedding == "tfidf" else embedding.capitalize()
            )
