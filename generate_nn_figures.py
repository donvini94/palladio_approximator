#!/usr/bin/env python3
"""
Generate thesis figures for Neural Network hyperparameter analysis.

This script creates consolidated plots showing the effect of each NN hyperparameter
...
# Neural Network hyperparameters to analyze
NN_PARAMS = {
...
def load_mlflow_data(experiment_name: str = None) -> pd.DataFrame:
    ...
    for exp_id in experiment_ids:
        runs = mlflow.search_runs(
            ...
            filter_string="params.model_type = 'torch'",
            max_results=10000,
        )
        if not runs.empty:
    ...
    if not all_runs:
        print("No Neural Network runs found in MLflow")
        return pd.DataFrame()

    df = pd.concat(all_runs, ignore_index=True)
    print(f"Loaded {len(df)} Neural Network experiment runs")
    return df
...
def extract_hyperparameter_from_run_name(run_name: str) -> Dict[str, str]:
    ...
    # Parse run name format: param=value_model=svm_emb=embedding_norm=normalization_runX
...
def clean_and_prepare_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Filter for Neural Network experiments only
    df = df[df["params.model_type"] == "torch"].copy()

    if df.empty:
        print("No Neural Network experiments found")
        return df
    ...
    numeric_params = ["learning_rate", "dropout_rate", "batch_size", "epochs"]
...
def create_consolidated_plot(
    ...
    if param in ["learning_rate", "dropout_rate", "batch_size", "epochs"]:
        # All NN parameters are numeric - sort numerically
        # Only use values that actually exist in the data
        actual_values = embedding_data[param_col].dropna().unique()
        all_param_values = sorted(actual_values)
        # Use log scale for learning rate which typically spans orders of magnitude
        use_log_scale = param == "learning_rate"
    else:
        # Should not happen for NN parameters, but keep as fallback
        all_param_values = sorted(embedding_data[param_col].dropna().unique())
        use_log_scale = False

    # Prepare data for plotting - only include parameter values with actual metric data
    valid_param_values = []
    norm_means = []
    norm_stds = []
    no_norm_means = []
    ...
    # All NN parameters are continuous, so use line plots
    if use_log_scale and param == "learning_rate":
        ...
        # Use positions for linear scale (dropout_rate, batch_size, epochs)
        if not all(np.isnan(norm_means)):
            ax.errorbar(
                x_positions,
                norm_means,
    ...
    ax.set_title(
        f"Effect of {NN_PARAMS[param]} on {metric_name}\n{embedding_str} Embedding",
        fontsize=14,
        fontweight="bold",
    )
    ax.set_xlabel(NN_PARAMS[param], fontsize=12)
    ...
    # Create filename
    filename = f"nn_{param}_{metric}_{embedding}_consolidated.png"
...
def create_summary_statistics(data: pd.DataFrame, output_dir: Path):
    ...
    if summary_stats:
        ...
        summary_df.to_csv(output_dir / "nn_summary_statistics.csv", index=False)
        print(f"Saved summary statistics: {len(summary_df)} rows")


def main():
    parser = argparse.ArgumentParser(
        description="Generate Neural Network hyperparameter analysis figures"
    )
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    print(f"Generating Neural Network analysis figures in: {output_dir}")
    ...
    print(
        ...
        f"{len(NN_PARAMS)} parameters, {len(available_embeddings)} embeddings "
    ...
    print(f"Summary statistics saved to nn_summary_statistics.csv")

    # Print organization suggestion
    print(f"\n=== Thesis Organization Suggestion ===")
    for param in NN_PARAMS.keys():
        print(f"\nSection: Effect of {NN_PARAMS[param]} on Prediction Quality")
        for embedding in available_embeddings:
            embedding_name = (
                embedding.upper() if embedding == "tfidf" else embedding.capitalize()
            )
