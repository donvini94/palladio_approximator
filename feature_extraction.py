...
import numpy as np
from transformers import AutoTokenizer, AutoModel, logging, AutoModelForCausalLM, BitsAndBytesConfig
import torch
...
def build_llama_features(
    ...
    if device == "cuda":
        ...
        if free_mem_gb < 10 and "7b" in model_name.lower():
            print("WARNING: Less than 10GB VRAM available. Model may not fit in memory.")
        elif free_mem_gb < 20 and "13b" in model_name.lower():
            print("WARNING: Less than 20GB VRAM available. Model may not fit in memory.")
    ...
    try:
        ...
        if use_4bit:
            ...
            quantization_config = BitsAndBytesConfig(
                load_in_4bit=True,
                bnb_4bit_compute_dtype=torch.float16 if use_half_precision else torch.float32,
            ...
            quantization_config = BitsAndBytesConfig(
                load_in_8bit=True,
                bnb_8bit_compute_dtype=torch.float16 if use_half_precision else torch.float32
            )
        ...
        if any(name in model_name.lower() for name in ["llama", "mistral", "codellama"]):
    ...
    try:
        ...
        if hasattr(model.config, "max_position_embeddings"):
            ...
            config_attrs = ["max_sequence_length", "seq_length", "n_positions", "max_seq_len"]
    ...
    def encode_with_llama(df):
        ...
        for i in tqdm(range(0, len(texts), actual_batch_size), desc=f"Encoding with {model_name}"):
            ...
            for text in batch_texts:
                ...
                if token_count > max_length:
                    ...
                    if not natural_breaks:
                        ...
                        target_char_length = int(max_length * char_ratio * 0.9)  # 90% to be safe
                        ...
                        while start_pos < len(text):
                            ...
                            valid_breaks = [b for b in natural_breaks 
                                           if b[0] > start_pos and b[0] <= target_pos + 1000]
                            
                            if valid_breaks:
                                # Prioritize code structure breaks over simple newlines
                                code_breaks = [b for b in valid_breaks if b[1] == "code_structure"]
                                if code_breaks:
                    ...
                    if len(chunks) > max_chunks:
                        print(f"Limiting document from {len(chunks)} to {max_chunks} chunks due to memory constraints")
                            ...
                            if amp_enabled:
                                with torch.amp.autocast("cuda"):
                                    if isinstance(model, AutoModelForCausalLM):
                                        ...
                                        embedding = torch.sum(hidden_states * mask, dim=1) / torch.sum(mask, dim=1)
        ...
        for i in tqdm(range(0, len(texts), batch_size), desc="Encoding DSL files"):
            ...
            for text in batch_texts:
                ...
                if token_count > max_length:
                    ...
                    for delimiter in ['}', ';', '\n\n']:
                        positions = [pos for pos in range(len(text)) if text.startswith(delimiter, pos)]
                    ...
                    if not natural_breaks:
                        print(f"No natural breaks found in document of {token_count} tokens, using token-based chunking")
                        ...
                        for i in range(0, token_count, effective_chunk_length):
                            ...
                            if end_idx < token_count and end_idx - i > sliding_window_overlap:
                                chunk_tokens = tokenized_text[i:end_idx]
                            else:
                        ...
                        while start_pos < len(text):
                            ...
                            valid_breaks = [b for b in natural_breaks if b[0] > start_pos and b[0] <= target_pos + 500]
                ...
                if len(chunks) > max_chunks_per_doc:
                    print(f"Warning: Limiting document from {len(chunks)} to {max_chunks_per_doc} chunks")
                    ...
                    if max_chunks_per_doc >= 3:
                        ...
                        middle_indices = [int(i * step) for i in range(num_middle_chunks)]
                        selected_middle = [middle_chunks[i] for i in middle_indices]
                ...
                if len(chunk_embeddings) > 1:
                    ...
                    if num_chunks >= 5:
                        ...
                        pos_weights[1] = 1.2  # Second chunk (often contains type declarations)
                    ...
                    non_ws_counts = np.array([sum(1 for char in c if not char.isspace()) for c in chunks])
                    ...
                    combined_weights = (0.3 * pos_weights + 0.4 * content_weights + 0.3 * density_weights)
                    ...
                    if len(embeddings) < 2:  # Only for the first document
                        ...
                        for i, w in enumerate(final_weights):
                            print(f"  Chunk {i}: weight={w:.3f}, length={len(chunks[i])}")
