...
def find_models_and_embeddings(data_dir="."):
    ...
    for model_file in model_files:
        ...
        for embedding_file in embedding_files:
            ...
            embedding_match = re.match(
                r"(.+?)_(.+?)_features_checkpoint\.pkl", embedding_basename
            )
...
def analyze_mlflow_experiments():
    ...
    print(
        "MLflow analysis complete. Check 'figures/performance' and 'figures/training' directories for results."
    )
...
def generate_model_predictions_analysis(pairs):
    ...
    results = []
    for model_file, embedding_file, model_type, embedding_type, prediction_mode in tqdm(
        pairs
    ):
        try:
    ...
    if results:
        ...
        print(
            f"Prediction analysis complete. Analyzed {len(results)} model configurations."
        )
    else:
...
def generate_embedding_visualizations(pairs):
    ...
    print(
        f"Embedding visualization complete. Visualized {len(embedding_types_processed)} embedding types."
    )
...
def generate_model_representations(pairs):
    ...
    for model_file, embedding_file, model_type, embedding_type, prediction_mode in tqdm(
        ...
        try:
            ...
            print(
                f"Error analyzing representations for {model_type} + {embedding_type}: {e}"
            )

    print(
        f"Model representation analysis complete. Analyzed {len(results)} model configurations."
    )
...
def compare_models(pairs):
    ...
    for (
        ...
        pairs_by_mode[prediction_mode].append(
            (model_file, embedding_file, model_type, embedding_type)
        )
    ...
    for mode, mode_pairs in pairs_by_mode.items():
        ...
        try:
            ...
            print(
                f"Compared {len(result.get('models_compared', []))} models for {mode} prediction mode."
            )
...
def analyze_attention_patterns(code_files_dir, model_name=None):
    ...
    if not os.path.exists(code_files_dir):
        print(
            f"Directory {code_files_dir} does not exist. Skipping attention analysis."
        )
        return
...
def generate_learning_curve_analysis(model_types, embedding_types):
    ...
    for model_type in model_types:
        for embedding_type in embedding_types:
            try:
                print(
                    f"Generating learning curves for {embedding_type} + {model_type}..."
                )

                # Generate learning curves
                df = generate_learning_curves(
                    model_type, embedding_type, train_sizes=train_sizes
                )
                ...
                print(
                    f"Error generating learning curves for {embedding_type} + {model_type}: {e}"
                )
...
def generate_comprehensive_thesis_visuals():
    ...
    generate_learning_curve_analysis(
        model_types=["rf", "torch"], embedding_types=["tfidf", "bert"]
    )

    print(
        "Comprehensive thesis visualization generation complete. Check the 'figures' directory."
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate visualizations and analysis for thesis"
    )

    parser.add_argument(
        "--all", action="store_true", help="Generate all thesis visualizations"
    )
    parser.add_argument(
        "--mlflow", action="store_true", help="Analyze MLflow experiments"
    )
    parser.add_argument(
        "--training_metrics",
        action="store_true",
    ...
    parser.add_argument(
        "--embeddings", action="store_true", help="Run embedding analysis"
    )
    parser.add_argument(
        "--attention", action="store_true", help="Analyze attention patterns"
    )
    parser.add_argument(
        "--code_dir",
        type=str,
    ...
    parser.add_argument(
        "--learning_curves", action="store_true", help="Generate learning curves"
    )
    ...
    if args.all:
        ...
        if args.training_metrics:
            ...
            try:
                ...
                if df is not None:
                    print(
                        f"Generated training metrics visualization with {len(df)} epochs."
                    )
                else:
                    print("No PyTorch training metrics found.")
            except Exception as e:
