...
def create_optuna_pruner(args):
    ...
    if args.model == "torch":
        ...
        return optuna.pruners.MedianPruner(
            n_startup_trials=10, n_warmup_steps=0, interval_steps=1
        )
...
def get_metric_value(results, metric_name):
    ...
    Args:
        results (dict or object): Dictionary or object with evaluation results
        metric_name (str): Name of the metric to extract (e.g., 'val_mse')

    Returns:
        float: The value of the specified metric
    """
    # Handle evaluation result objects with .metrics attribute
...
def optimize_rf_hyperparameters(
    X_train, y_train, X_val, y_val, n_trials=30, metric="val_mse"
):
    """
    ...
    # Optimize
    logger.info(
        f"Starting Random Forest hyperparameter optimization with {n_trials} trials"
    )
...
def optimize_linear_hyperparameters(
    X_train, y_train, X_val, y_val, model_type, n_trials=30, metric="val_mse"
):
    """
    ...
    # Optimize
    logger.info(
        f"Starting {model_type.capitalize()} hyperparameter optimization with {n_trials} trials"
    )
    start_time = time.time()
    study.optimize(objective, n_trials=n_trials)
    elapsed_time = time.time() - start_time
...
def optimize_torch_hyperparameters(
    X_train, y_train, X_val, y_val, device, n_trials=30, metric="val_mse"
):
    """
    ...
    def objective(trial):
        ...
        first_layer_size = trial.suggest_categorical(
            "first_layer_size", [256, 512, 768, 1024]
        )
        ...
        for i in range(1, n_layers):
            ...
            next_size = trial.suggest_int(
                f"layer_{i}_size", max(64, prev_size // 4), prev_size
            )
        ...
        try:
            ...
            if device == "cuda":
                if (
                    hasattr(model, "training_metrics")
                    and "pytorch_model" in model.training_metrics
                ):
...
def optimize_architecture_hyperparameters(
    ...
    Args:
        ...
        X_val: Validation features
        y_val: Validation labels
        architecture_type: Type of architecture to optimize
        n_trials: Number of optimization trials
        metric: Metric to optimize ('val_mse', 'val_mae', 'val_r2')
    ...
    def objective(trial):
        ...
        if architecture_type == "standard":
            ...
            params = {
                "hidden_dims": [
                    trial.suggest_categorical("layer1_size", [512, 768, 1024]),
                    trial.suggest_categorical("layer2_size", [256, 512, 768]),
                    trial.suggest_categorical("layer3_size", [128, 256, 512]),
                    trial.suggest_categorical("layer4_size", [64, 128, 256]),
                ],
        ...
        elif architecture_type == "adaptive":
            params = {
                "base_hidden_dims": [
                    trial.suggest_categorical("layer1_size", [256, 512, 768]),
                    trial.suggest_categorical("layer2_size", [128, 256, 512]),
                    trial.suggest_categorical("layer3_size", [64, 128, 256]),
                ],
                "complexity_threshold": trial.suggest_float(
                    "complexity_threshold", 0.3, 0.7
                ),
                "dropout_rate": trial.suggest_float("dropout_rate", 0.1, 0.5),
            }
        else:
            # Default parameters for any other architectures
            params = {
                "dropout_rate": trial.suggest_float("dropout_rate", 0.1, 0.5),
            }
    ...
    logger.info(
        f"Starting {architecture_type} architecture hyperparameter optimization with {n_trials} trials"
    )
    start_time = time.time()
    study.optimize(objective, n_trials=n_trials, catch=(RuntimeError, ValueError))
    elapsed_time = time.time() - start_time
    logger.info(
        f"Architecture hyperparameter optimization completed in {elapsed_time:.2f} seconds"
    )
...
def run_hyperparameter_optimization(args, X_train, y_train, X_val, y_val):
    ...
    metric = (
        args.optimization_metric if hasattr(args, "optimization_metric") else "val_mse"
    )
    ...
    if args.use_mlflow:
        ...
        try:
            ...
            importance_plot = (
                f"models/optimization/{args.model}_importance_{timestamp}.png"
            )
