...
def normalize_attention(attention_matrix):
    ...
    normalized = (attention_matrix - attention_matrix.min()) / (
        attention_matrix.max() - attention_matrix.min()
    )
    return normalized
...
def visualize_attention_patterns(
    ...
    is_causal_lm = any(
        name in model_name.lower() for name in ["llama", "gpt", "falcon", "mistral"]
    )
    ...
    try:
        ...
        if is_causal_lm:
            ...
            # BERT-like models
            model = AutoModel.from_pretrained(model_name, output_attentions=True).to(
                device
            )
        ...
        for layer_idx in layer_indices:
            if layer_idx < len(attentions):
                ...
                plt.savefig(
                    os.path.join(output_dir, f"{prefix}layer_{layer_idx}_avg.png"),
                    dpi=300,
                    bbox_inches="tight",
                )
                plt.close()
        ...
        last_layer_attention = (
            attentions[last_layer_idx][0].mean(dim=0).cpu().numpy()
        )  # Average across heads

        # Get attention importance for each token
        token_importance = last_layer_attention.mean(
            axis=0
        ...
        sns.barplot(
            data=token_df.head(30), x="Importance", y="Token", palette="viridis"
        )
        ...
        token_df.to_csv(
            os.path.join(output_dir, f"{prefix}token_importance.csv"), index=False
        )
...
def analyze_model_representations(
    model_path, embedding_path, output_dir="figures/embeddings", n_components=2
):
    """
    ...
    try:
        ...
        match_embedding = re.match(
            r"(.+?)_(.+?)_features_checkpoint\.pkl", embedding_file
        )
        ...
        if match_model and match_embedding:
            ...
            scatter = plt.scatter(
                X_pca[:, 0], X_pca[:, 1], c=errors, cmap="viridis", alpha=0.7
            )
            plt.colorbar(scatter, label="Prediction Error")
            plt.title(
                f"PCA of {embedding_type.upper()} Embeddings (Colored by {model_type.upper()} Model Error)"
            )
            ...
            plt.savefig(
                os.path.join(
                    output_dir, f"{embedding_type}_{model_type}_pca_error.png"
                ),
                dpi=300,
            )
            plt.close()
            ...
            plt.figure(figsize=(10, 8))
            scatter = plt.scatter(
                X_tsne[:, 0], X_tsne[:, 1], c=errors, cmap="viridis", alpha=0.7
            )
            plt.colorbar(scatter, label="Prediction Error")
            plt.title(
                f"t-SNE of {embedding_type.upper()} Embeddings (Colored by {model_type.upper()} Model Error)"
            )
            ...
            plt.savefig(
                os.path.join(
                    output_dir, f"{embedding_type}_{model_type}_tsne_error.png"
                ),
                dpi=300,
            )
            plt.close()
            ...
            plt.figure(figsize=(10, 8))
            scatter = plt.scatter(
                X_pca[:, 0], X_pca[:, 1], c=y_values, cmap="plasma", alpha=0.7
            )
            plt.colorbar(scatter, label="Actual Value")
            plt.title(
                f"PCA of {embedding_type.upper()} Embeddings (Colored by Actual Value)"
            )
            plt.xlabel("PCA Component 1")
            plt.ylabel("PCA Component 2")
            plt.tight_layout()
            plt.savefig(
                os.path.join(
                    output_dir, f"{embedding_type}_{model_type}_pca_actual.png"
                ),
                dpi=300,
            )
            plt.close()
            ...
            plt.figure(figsize=(10, 8))
            scatter = plt.scatter(
                X_tsne[:, 0], X_tsne[:, 1], c=y_values, cmap="plasma", alpha=0.7
            )
            plt.colorbar(scatter, label="Actual Value")
            plt.title(
                f"t-SNE of {embedding_type.upper()} Embeddings (Colored by Actual Value)"
            )
            plt.xlabel("t-SNE Component 1")
            plt.ylabel("t-SNE Component 2")
            plt.tight_layout()
            plt.savefig(
                os.path.join(
                    output_dir, f"{embedding_type}_{model_type}_tsne_actual.png"
                ),
                dpi=300,
            )
            plt.close()
            ...
            if model_type == "rf" and hasattr(model, "feature_importances_"):
                ...
                if len(importances) > 50:
                    ...
                    plt.yticks(
                        range(len(top_indices)), [f"Feature {i}" for i in top_indices]
                    )
                    plt.title(
                        f"Top 50 Important Features ({embedding_type.upper()} with {model_type.upper()})"
                    )
                else:
                    ...
                    plt.title(
                        f"Feature Importances ({embedding_type.upper()} with {model_type.upper()})"
                    )
                ...
                plt.savefig(
                    os.path.join(
                        output_dir,
                        f"{embedding_type}_{model_type}_feature_importance.png",
                    ),
                    dpi=300,
                )
                plt.close()
...
def compare_model_predictions(
    model_paths, embedding_paths, output_dir="figures/model_comparison"
):
    """
            ...
            if match_model and match_embedding:
                ...
                if i > 0:
                    ...
                    if embeddings["X_test"].shape != X_test.shape:
                        print(
                            f"Skipping {model_type} + {embedding_type} due to shape mismatch"
                        )
                        continue
    ...
    plt.figure(figsize=(10, 8))
    sns.heatmap(
        correlation, annot=True, cmap="viridis", vmin=-1, vmax=1, linewidths=0.5
    )
    ...
    for i, (pred, name) in enumerate(zip(preds, model_names)):
        ...
        metrics.append({"model": name, "mse": mse, "mae": mae})
    ...
    # Save metrics to CSV
    metrics_df.to_csv(
        os.path.join(output_dir, "model_comparison_metrics.csv"), index=False
    )
...
def extract_token_attention_patterns(
    file_path, model_name=None, keywords=None, output_dir="figures/token_patterns"
):
    """
    ...
    try:
        ...
        for keyword in keywords:
            ...
            indices = [
                i for i, token in enumerate(tokens) if keyword.lower() in token.lower()
            ]
            if indices:
        ...
        for keyword, indices in keyword_indices.items():
            ...
            token_attention_df.to_csv(
                os.path.join(
                    output_dir,
                    f"{file_name}_{model_short_name}_{keyword}_attention.csv",
                ),
                index=False,
            )
            ...
            from_df = token_attention_df.sort_values(
                f"From_{last_layer_idx}", ascending=False
            )
            ...
            plt.savefig(
                os.path.join(
                    output_dir, f"{file_name}_{model_short_name}_{keyword}_from.png"
                ),
                dpi=300,
            )
            plt.close()
            ...
            to_df = token_attention_df.sort_values(
                f"To_{last_layer_idx}", ascending=False
            )
            ...
            plt.savefig(
                os.path.join(
                    output_dir, f"{file_name}_{model_short_name}_{keyword}_to.png"
                ),
                dpi=300,
            )
            plt.close()
            ...
            plt.savefig(
                os.path.join(
                    output_dir,
                    f"{file_name}_{model_short_name}_{keyword}_layer_progression.png",
                ),
                dpi=300,
            )
            plt.close()
...
def batch_process_files(
    directory, pattern="*.tpcm", model_name=None, output_dir="figures/batch_analysis"
):
    """
    ...
    for file_path in tqdm(files, desc="Processing files"):
        try:
            ...
            result = visualize_attention_patterns(
                file_path, model_name=model_name, output_dir=file_output_dir
            )
...
if __name__ == "__main__":
    ...
    if sample_files:
        ...
        # Visualize patterns
        result = visualize_attention_patterns(
            sample_file, model_name="microsoft/codebert-base"
        )

        print(
            f"Attention visualization complete. Check the 'figures/attention' directory."
        )
    else:
